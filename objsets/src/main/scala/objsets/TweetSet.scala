...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet ={
    val mostRetweetedInLeft  = left.mostRetweetedAcc(acc)
    val mostRetweetedInRight = right.mostRetweetedAcc(acc)
    List(mostRetweetedInLeft, mostRetweetedInRight, elem).maxBy(_.retweets)
  }
